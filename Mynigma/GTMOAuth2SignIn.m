...
finishedWithFetcher:(GTMOAuth2Fetcher *)fetcher
  ...
  else if (error == nil && [self.authentication.serviceProvider isEqual:@"Outlook"])
  {
    // fetch the user's information from the Microsoft server
    [self fetchOutlookUserInfo];
  }
  else
  {
...
+ (GTMOAuth2Fetcher *)userInfoFetcherWithAuth:(GTMOAuth2Authentication *)auth {
  // create a fetcher for obtaining the user's email or profile
    NSURL *infoURL;
    if ([auth.serviceProvider isEqual:@"Outlook"])
    {
        infoURL = [NSURL URLWithString:@"https://apis.live.net/v5.0/me"];
    }
    else
    {
        infoURL = [[self class] googleUserInfoURL];
    }
...
/* Added by Lukas 2015-01-10 */

- (void) fetchOutlookUserInfo
{
    if (!self.shouldFetchGoogleUserProfile) {
...
- (void)infoFetcher:(GTMOAuth2Fetcher *)fetcher
  ...
  if (error) {
    ...
    if ([auth.serviceProvider isEqual:@"Outlook"])
        [self updateOutlookUserInfoWithData:data];
    else
...
- (void)updateOutlookUserInfoWithData:(NSData *)data {
    if (!data) return;
    ...
    if (profileDict) {
        ...
        NSString *subjectID = [profileDict objectForKey:@"id"];
        [auth setUserID:subjectID];
        
        // Save the email into the auth object
        NSString *email = [[profileDict objectForKey:@"emails"] objectForKey:@"account"];
