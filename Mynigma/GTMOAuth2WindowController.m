...
- (void)awakeFromNib {
  ...
  if ([html length] > 0) {
   ...
   // hide it, we always safe the refreshToken
  // hide the keychain checkbox if we're not supporting keychain
    BOOL hideKeychainCheckbox = YES;  //![self shouldUseKeychain];
    ...
    if (error == nil) {
      ...
      if (shouldUseKeychain) {
        BOOL canAuthorize = auth.canAuthorize;
          ...
          //we always want to use the keychain
          BOOL isKeychainChecked = YES;//([keychainCheckbox_ state] == NSOnState);
...
- (void)webView:(WebView *)sender resource:(id)identifier didFinishLoadingFromDataSource:(WebDataSource *)dataSource {
  ...
  if ([title length] > 0) {
    ...
    /* Edited by Lukas 2015-01-10 */
    if([[self.authentication serviceProvider] isEqual:@"Google"])
    {
        // Set the known email address (google)
        [sender stringByEvaluatingJavaScriptFromString:[NSString stringWithFormat:@"document.getElementById('Email').value = '%@';", _emailAddress]];
        [sender stringByEvaluatingJavaScriptFromString:@"document.getElementById('Passwd').focus();"];
    }
    else
    {
        // Set the known email address (outlook)
        [sender stringByEvaluatingJavaScriptFromString:[NSString stringWithFormat:@"document.getElementById('i0116').value = '%@';", _emailAddress]];
        [sender stringByEvaluatingJavaScriptFromString:@"document.getElementById('Passwd').focus();"];
    }
    /* End */
...
//adjust the height and width to the content of the webView's scrollView

NSString *documentHeight = [sender stringByEvaluatingJavaScriptFromString:@"document.body.scrollHeight;"];
NSString *documentWidth = [sender stringByEvaluatingJavaScriptFromString:@"document.body.scrollWidth;"];

    CGFloat neededHeight = documentHeight.floatValue + 20; //add 20 for the bottom bar
    CGFloat neededWidth = documentWidth.floatValue;
    
    NSSize screenSize = [NSScreen mainScreen].frame.size;
    
    if(neededHeight > screenSize.height - 200)
        neededHeight = screenSize.height - 200;

    if(neededHeight > screenSize.width - 200)
        neededHeight = screenSize.width - 200;

    if(neededWidth < 10)
        neededWidth = self.window.frame.size.width;
    
    NSRect windowFrame = self.window.frame;
  
    CGFloat topEdge = windowFrame.origin.y + windowFrame.size.height;
    
    CGFloat newBottomEdge = topEdge - neededHeight;
    
    if(newBottomEdge < 100)
    {
        neededHeight -= (100 - newBottomEdge);
        newBottomEdge = 100;
    }
    
    CGFloat horizontalCenter = windowFrame.origin.x + windowFrame.size.width/2.;
    
    CGFloat newLeftEdge = horizontalCenter - neededWidth/2.;
    
    NSRect newFrame = NSMakeRect(newLeftEdge, newBottomEdge, neededWidth, neededHeight);
                            ...
                            authentication:(GTMOAuth2Authentication *)auth
{
    ...
    // Overwriting keychain name. We want the used email address in it.
    keychainItemName = [NSString stringWithFormat:@"Mynigma OAuth: %@", auth.userEmail];
...
{
    //always use keychain
    //keychainItemName will be set later
    return YES;
