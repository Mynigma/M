    ...
    if (!self.view.hidden) {
      ...
      if (popViewBlock) {
        ...
        self.popViewBlock = nil;
          ...
          //no longer needed - the Setup_SeparateController will take care of dimissing the popover...
          
//          UINavigationController* navController = (UINavigationController*)self.presentingViewController;
//          
//         if([navController isKindOfClass:[UINavigationController class]])
//          {
//              UIViewController* presentingOAuthController = [[navController viewControllers] firstObject];
//              
//              if([presentingOAuthController isKindOfClass:[OAuthController class]])
//              {
//                  [(OAuthController*)presentingOAuthController returnWithAuthentication:[self authentication]];
//                  
//                  return;
//              }
//          }
...
- (void)webViewDidFinishLoad:(UIWebView *)webView {
    ...
    //set the preferred content size to the size of the webpage's content
    //this causes adjustment of popover content size
    CGSize contentSize = webView.scrollView.contentSize;
    
    if(contentSize.width == 0 || contentSize.height == 0)
        contentSize = CGSizeMake(400, 600);
    
    [self setPreferredContentSize:contentSize];
  ...
  if ([title length] > 0) {
    ...
    [webView stringByEvaluatingJavaScriptFromString:[NSString stringWithFormat:@"document.getElementById('Email').value = '%@';", _emailAddress]];
    //[webView stringByEvaluatingJavaScriptFromString:@"document.getElementById('Passwd').focus();"];
    }
    else
    {
    // Set the known email address (outlook)
        [webView stringByEvaluatingJavaScriptFromString:[NSString stringWithFormat:@"document.getElementById('i0116').value = '%@';", _emailAddress]];
    //[webView stringByEvaluatingJavaScriptFromString:@"document.getElementById('Passwd').focus();"];
    }
    /* End */
