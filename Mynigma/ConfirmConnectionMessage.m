...
@implementation ConfirmConnectionMessage

+ (DeviceMessage*)confirmConnectionMessageWithSecretKeyData:(NSData*)secretKeyData inThread:(NSString*)threadID senderDevice:(MynigmaDevice*)senderDevice targetDevice:(MynigmaDevice*)targetDevice onLocalContext:(NSManagedObjectContext*)localContext isResponse:(BOOL)isAcknowledgement
{
    ...
    if(isAcknowledgement)
        [newMessage setMessageCommand:@"1_ACK_CONFIRM_CONNECTION"];
    else
        [newMessage setMessageCommand:@"1_CONFIRM_CONNECTION"];

    [newMessage setPayload:@[secretKeyData]];
